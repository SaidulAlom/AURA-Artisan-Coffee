...
export default function BrewingGuides() {
  ...
  return (
    <section id="guides-section" className="py-20 lg:py-28 bg-coffee-milk relative border-t border-coffee-latte">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-coffee-latte border border-coffee-caramel text-xs font-bold text-coffee-espresso">
            ...
            <button
              ...
              className={`px-5 py-3 rounded-2xl text-xs font-bold whitespace-nowrap transition-all duration-200 flex items-center gap-2.5 ${
                activeMethodId === method.id
                  ? 'bg-coffee-roasted text-white shadow-md scale-105'
        ...
        <motion.div
          ...
          className="bg-coffee-latte rounded-3xl p-6 sm:p-10 soft-shadow-lg border border-coffee-milk grid grid-cols-1 lg:grid-cols-12 gap-8 items-start"
        >
          
          {/* Left Column: Method Specifications & Interactive Calculator */}
          <div className="lg:col-span-5 space-y-6">
            <div className="bg-coffee-milk rounded-2xl p-6 soft-shadow space-y-4 border border-coffee-latte">
              
              <div className="flex items-center justify-between border-b border-coffee-latte pb-4">
                <div>
                  <span className="text-[11px] font-bold text-coffee-roasted uppercase tracking-wider block">
                    Brewing Method
                  </span>
                  <h3 className="font-serif text-2xl font-extrabold text-coffee-espresso">
                    {activeMethod.name}
                  </h3>
                </div>
                <div className="w-12 h-12 rounded-2xl bg-coffee-caramel/20 flex items-center justify-center text-coffee-roasted">
                  <Droplets className="w-6 h-6" />
                </div>
              </div>

              <p className="text-xs text-coffee-espresso/80 leading-relaxed">
                {activeMethod.description}
              </p>

              {/* Water Ratio Interactive Calculator */}
              <div className="bg-coffee-latte p-4 rounded-xl space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-coffee-espresso flex items-center gap-1.5">
                    <Scale className="w-4 h-4 text-coffee-roasted" />
                    <span>Coffee Dose (Grams)</span>
                  </span>
                  <span className="font-serif font-extrabold text-sm text-coffee-roasted">
                    {customCoffeeGrams}g
                  </span>
                </div>
                ...
                <input
                  ...
                  className="w-full accent-coffee-roasted cursor-pointer"
                />

                <div className="grid grid-cols-2 gap-2 pt-2 text-center text-xs">
                  <div className="bg-coffee-milk p-2.5 rounded-lg border border-coffee-espresso/5">
                    <span className="text-[10px] text-coffee-espresso/60 block font-semibold">Water Needed</span>
                    <span className="font-bold text-coffee-espresso text-sm">{calculatedWater} {activeMethod.id === 'espresso' ? 'g yield' : 'ml'}</span>
                  </div>
                  <div className="bg-coffee-milk p-2.5 rounded-lg border border-coffee-espresso/5">
                    <span className="text-[10px] text-coffee-espresso/60 block font-semibold">Target Ratio</span>
                    <span className="font-bold text-coffee-roasted text-sm">{activeMethod.ratio}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                <div className="p-3 rounded-xl bg-coffee-latte">
                  <span className="text-[10px] text-coffee-espresso/60 font-semibold block">Grind Size</span>
                  <span className="font-bold text-coffee-espresso">{activeMethod.grindSize}</span>
                </div>

                <div className="p-3 rounded-xl bg-coffee-latte">
                  <span className="text-[10px] text-coffee-espresso/60 font-semibold block">Water Temp</span>
                  <span className="font-bold text-coffee-espresso">{activeMethod.waterTemp}</span>
                </div>
              </div>

              {/* Built-in Stopwatch Barista Companion */}
              <div className="p-4 rounded-xl bg-coffee-espresso text-white flex items-center justify-between">
                <div>
                  <span className="text-[10px] text-coffee-caramel font-extrabold uppercase tracking-wider block">
                    Barista Brew Timer
                  </span>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="p-2.5 rounded-full bg-coffee-roasted text-white hover:bg-coffee-caramel hover:text-coffee-espresso transition-colors"
                  >
                    {isTimerRunning ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4" />}
                  </button>
                  <button
                    onClick={resetTimer}
                    className="p-2.5 rounded-full bg-coffee-milk/20 text-white hover:bg-coffee-milk/40 transition-colors"
                  >
          ...
          <div className="lg:col-span-7 space-y-4">
            <h4 className="font-serif text-xl font-bold text-coffee-espresso flex items-center gap-2">
              <Clock className="w-5 h-5 text-coffee-roasted" />
                ...
                return (
                  <motion.div
                    ...
                    className={`p-5 rounded-2xl transition-all cursor-pointer border ${
                      isCurrent
                        ? 'bg-coffee-milk soft-shadow-lg border-coffee-roasted scale-[1.01]'
                        : 'bg-coffee-milk/70 hover:bg-coffee-milk border-transparent'
                    }`}
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h5 className="font-serif text-base font-bold text-coffee-espresso">
                            {step.title}
                          </h5>
                          <span className="text-[11px] font-semibold text-coffee-roasted">
                            {step.duration}
                          </span>
                        </div>
                      </div>
                      ...
                      {isCurrent && (
                        <CheckCircle2 className="w-5 h-5 text-coffee-roasted shrink-0" />
                      )}
                    </div>

                    <p className="text-xs text-coffee-espresso/90 mt-2 leading-relaxed font-normal pl-10">
                      {step.instruction}
                    </p>

                    {step.tip && (
                      <div className="mt-2 ml-10 p-2.5 rounded-lg bg-coffee-latte text-[11px] text-coffee-espresso font-medium border-l-2 border-coffee-caramel">
